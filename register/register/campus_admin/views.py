...
def course_list(request,pk,pk1,pk2,pk4):
   ...
   try:
        ...
        sem = OfferedIn.objects.get(semester=pk2)
        students = Registers.objects.filter(studentId=stud,semesterNo=sem)
   except:
        students = None
   args = {'students':students,'pk1':pk1,'pk2':pk2,'pk4':pk4}
...
def result_add(request,pk,pk1,pk2,pk3,pk4):
    stud = student.objects.get(student_id=pk)
    sem = OfferedIn.objects.get(semester=pk2)
    corse = course.objects.get(courseNo=pk3)
    students = Registers.objects.get(studentId=stud,courseNo=corse,semesterNo=sem)
    form = RegistersForm(request.POST or None,instance=students)

    if form.is_valid():
        temp = form.save(commit=False)
        grade = request.POST.get('grade')
        Registers.objects.filter(studentId=stud, courseNo=corse).update(grade=grade)
        # temp.save()
...
def success_fee_receipt(request, pk4, pk1, pk, pk2):
...
def fee_receipt_add(request,pk,pk1,pk2,pk4):
    sem = OfferedIn.objects.get(semester=pk2)
    current_student = student.objects.get(student_id=pk)
    form = FeeReceiptForm(request.POST or None, instance=current_student)
    ...
    if form.is_valid():
        ...
        FeeReceipt.objects.filter(studentId=current_student,semesterNo=sem).update(receiptId=data)
        FeeReceipt.objects.filter(studentId=current_student,semesterNo=sem).update(status=data1)
...
def result_view(request,pk,pk1,pk2,pk4):
    ...
    #print(sem)
    current_student = student.objects.get(student_id=pk)
    temporary = Result.objects.get(studentId=current_student, semesterNo=sem)
    course = Registers.objects.filter(studentId=current_student, semesterNo=sem)
    total = 0
    CPI = 0.0
    ...
    if temporary.SPI == None:
        ...
        SPI = (math.ceil(calculate_points(current_student,sem,total)*100)/100)
        # print(SPI)
        temporary = Result.objects.get(studentId=current_student,semesterNo=sem)
        temporary.SPI = SPI
        temporary.save()
...
def fee_receipt_view(request,pk,pk1,pk2,pk4):
    ...
    sem = OfferedIn.objects.get(semester=pk2)
    current_student = student.objects.get(student_id=pk)
    print(sem, current_student)
    fee = FeeReceipt.objects.get(studentId=current_student,semesterNo=sem)
    iterate = int(pk2) - 1
